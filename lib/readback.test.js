...
test('byte identity is still enforced on a chunked response', async () => {
  ...
  try {
    await assert.rejects(
      () => verifyReadback(base, 'barcelona', PAYLOAD, { attempts: 1 }),
...
test('retries a stale read and succeeds once the blob propagates', async () => {
  // Netlify Blobs is eventually consistent: the first read after a write can
  // return the PREVIOUS image (runs #14014/#14016/#14017). Serve the old
  // bytes twice, then the new ones.
  const stale = Buffer.from('\x89PNG\r\n\x1a\n' + 'o'.repeat(2048), 'binary');
  let hits = 0;
  const server = http.createServer((req, res) => {
    hits += 1;
    const body = hits <= 2 ? stale : PAYLOAD;
  ...
  try {
    const out = await verifyReadback(base, 'barcelona', PAYLOAD, { attempts: 4, delayMs: 5 });
    assert.strictEqual(out.bytes, PAYLOAD.length);
    assert.strictEqual(hits, 3, 'should have retried twice before matching');
  } finally { server.close(); }
});

test('gives up if the blob never propagates', async () => {
  const stale = Buffer.from('\x89PNG\r\n\x1a\n' + 'o'.repeat(2048), 'binary');
  const { server, base } = await startServer(stale);
  try {
    await assert.rejects(
      () => verifyReadback(base, 'barcelona', PAYLOAD, { attempts: 3, delayMs: 5 }),
      /read-back bytes differ/);
  } finally { server.close(); }
});

test('does NOT retry a non-stale failure', async () => {
  // A 404 will not fix itself by waiting; burning the budget just delays
  // the real error.
  let hits = 0;
  const server = http.createServer((req, res) => { hits += 1; res.writeHead(404); res.end(); });
  await new Promise((r) => server.listen(0, '127.0.0.1', r));
  const base = `http://127.0.0.1:${server.address().port}`;
  try {
    await assert.rejects(
      () => verifyReadback(base, 'barcelona', PAYLOAD, { attempts: 4, delayMs: 5 }),
      /HTTP 404/);
    assert.strictEqual(hits, 1, 'must fail fast, not retry');
  } finally { server.close(); }
});
