...
//     OF THE TICK -- not of the moment the runner happens to evaluate it.
//     See tickOf() below: the runner calls this 40-135s after the cron fired,
//     and reading the raw clock then is exactly how both of this module's
//     silent-skip bugs happened.
...
/**
 * The cron tick `now` belongs to: `now` floored to the TICK_MINUTES grid.
 *
 * The runner does not evaluate the schedule at the tick. It evaluates it
 * when its pipeline step starts, which is the tick plus Netlify's dispatch
 * latency (18-50s, worst on the :00 slot) plus runner setup (checkout, npm,
 * Playwright, ImageMagick: 25-85s). That routinely lands in the NEXT minute.
 * Reading the raw clock then gates on the wrong minute: 11:51:38 is minute
 * 51, and 51 % 10 !== 0, so the 11:50 publish was silently skipped. On the
 * :00 slot it happened almost every time -- including every hourly quiet
 * run overnight, which needs minute === 0 and never saw it.
 *
 * Flooring makes the answer independent of runner speed for anything up to
 * a full tick late. Later than that cannot happen in practice: the next
 * dispatch cancels the run (concurrency: cancel-in-progress).
 *
 * This is the same bug as the pre-band version of this module, mirrored.
 * That one gated on getUTCMinutes() === 0 while the cron fired at :59, and
 * worked only while the runner was SLOW enough to spill into :00. This one
 * worked only while the runner was FAST enough not to spill at all. Neither
 * clock reading was ever the tick.
 *
 * @param {Date} now
 * @returns {Date}
 */
function tickOf(now) {
  ...
  return new Date(Math.floor(now.getTime() / tickMs) * tickMs);
}
 ...
 * Evaluated at the tick `now` belongs to (tickOf), so a late caller sees the
 * same band the tick did.
 *
...
function intervalMinutes(city, now = new Date()) {
  ...
  if (isQuiet(tz, quiet, tick)) return QUIET_INTERVAL_MINUTES;

  const { hour, minute } = getLocalHourMinute(tz, tick);
  const nowMinutes = hour * 60 + minute;
 ...
 *
 * `now` may be any instant inside the tick -- the runner passes its own
 * clock, a minute or more after the cron fired. It is floored to the tick
 * first (tickOf); never compare the raw clock's minute.
 *
...
function shouldRun(city, now = new Date()) {
  const tick = tickOf(now);
  const every = intervalMinutes(city, tick);
  return tick.getUTCMinutes() % every === 0;
}
